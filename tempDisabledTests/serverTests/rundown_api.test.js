...
import {
  formatEvent,
  getEventforDateforSport,
  getEventsforFutureDays,
  updateSportQueryDate,
...
beforeEach((done) => {
  Event.sync({ force: true })
    ...
    .then(() =>
      Promise.all(preloadedEventForTest.map((event) => Event.create(event)))
    )
    .then(async () => {
      await flushPromise();
      return done();
    ...
    .catch((e) =>
      console.log("Try again. Error setting up DB in rundown_api.test.js.", e)
    );
});
...
test("it calls Rundown API for event information", async () => {
  ...
  const options = {
    uri:
      "https://therundown-therundown-v1.p.rapidapi.com/sports/4/events/2019-05-12?include=scores",
    headers: {
...
test("it should update last sport query date for an event", async (done) => {
  ...
  Sport.findOne({ raw: true, where: { id: 1 } }).then(({ lastQueryDate }) => {
    expect(lastQueryDate.toString()).toBe(
      "Sun Jul 21 2019 23:00:00 GMT-0500 (Central Daylight Time)"
    );
    done();
  });
});
